    ...
    while true do
        ...
        for k,v in pairs(Config.Zones) do
            if GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < Config.DrawDistance and k == 'Pimp' then
                letSleep = false
                DrawText3Ds(v.Pos.x, v.Pos.y, v.Pos.z+1.0, "~b~[E]~w~ commander une pute")
...
RegisterNUICallback("ChooseCathrine", function (data, callback)
    ...
    exports['mythic_notify']:SendAlert('inform', "Cathrine est marquée sur votre GPS, allez la chercher.")
...
RegisterNUICallback("ChooseTatiana", function (data, callback)
    ...
    exports['mythic_notify']:SendAlert('inform', "Tatiana est marquée sur votre GPS, allez la chercher.")
    OnRouteToHooker = true
end)
...
RegisterNUICallback("ChooseBootylicious", function (data, callback)
    ...
    exports['mythic_notify']:SendAlert('inform', "Bootylicious est marquée sur votre GPS, allez la chercher.")
    OnRouteToHooker = true
end)
...
RegisterNUICallback("ChooseVennesa", function (data, callback)
    ...
    exports['mythic_notify']:SendAlert('inform', "Vennesa est marquée sur votre GPS, allez la chercher.")
    OnRouteToHooker = true
end)
...
AddEventHandler("sawu_hookers:ChosenHooker", function(model)
    if HookerSpawned then
        exports['mythic_notify']:SendAlert('error', "Vous avez déjà choisi une pute! Tu veux pas non plus faire une partouz ?")
    else
        HookerSpawned = true
        CreateHooker(model)
        Citizen.CreateThread(function()
                ...
                if OnRouteToHooker then 
                    if GetDistanceBetweenCoords(Coords, Config.Hookerspawns[spawn].x,Config.Hookerspawns[spawn].y,Config.Hookerspawns[spawn].z, true) < Config.DrawMarker then
                        ...
                        if GetPedInVehicleSeat(vehicle, -1) and IsPedInVehicle(ped, vehicle, true) and IsVehicleSeatFree(vehicle, 0) and not IsVehicleSeatFree(vehicle, -1) then
                            DrawText3Ds(Config.Hookerspawns[spawn].x,Config.Hookerspawns[spawn].y,Config.Hookerspawns[spawn].z+1.0, '[~b~E~w~] Pour signaler la pute')
                ...
                if HookerInCar then
                    ...
                    if GetPedInVehicleSeat(vehicle, -1) and IsPedInVehicle(ped, vehicle, true) and not IsVehicleSeatFree(vehicle, 0) and not IsVehicleSeatFree(vehicle, -1) then
                        ...
                        if IsVehicleStopped(vehicle) then
                            DrawText3Ds(Coords.x, Coords.y, Coords.z+1.0, '[~b~E~w~] Pour les services. [~r~H~w~] Pour la degager.')
                            if IsControlJustPressed(0, Keys["E"]) then
                            ...
                            DrawText3Ds(Coords.x, Coords.y, Coords.z+1.0, 'Conduire dans un endroit sûr (cacher)')
                        end
                    end
                end
                if letSleep then
